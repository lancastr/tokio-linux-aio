...
impl AioWaitFuture {
    fn return_request_to_pool(&mut self) {
        ...
        self.inner_context.requests.lock().return_in_flight_to_ready(req);
...
impl Future for AioWaitFuture {
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        if self.request.is_some() {
            self.result = Some(
                ready!(Pin::new(&mut self.rx).poll(cx))
                    .map_err(|_| AioCommandError::AioStopped)?,
            );
...
impl Drop for AioWaitFuture {
    fn drop(&mut self) {
        ...
        if let Some(in_flight) = self.request.take() {
            self.inner_context.requests.lock().move_to_outstanding(in_flight)
        }
    }
}