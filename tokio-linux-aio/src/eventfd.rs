...
impl EventFd {
    pub fn create(init: usize, semaphore: bool) -> Result<EventFd, EventFdError> {
        let mut flags = EfdFlags::EFD_CLOEXEC | EfdFlags::EFD_NONBLOCK;

        if semaphore {
            flags.insert(EfdFlags::EFD_SEMAPHORE);
        }

        let fd = eventfd(init as libc::c_uint, flags).map_err(EventFdError::CreateError)?;
...
mod tests {
    ...
    async fn non_semaphore() {
        ...
        let mut efd = EventFd::create(init, false).unwrap();
    ...
    async fn semaphore() {
        ...
        let mut efd = EventFd::create(init, true).unwrap();
